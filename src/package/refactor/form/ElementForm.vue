<template>
  <div class="panel-tab__content">
    ...
    <div class="element-property list-property">
      <el-divider><i class="el-icon-coin" /> 表单字段</el-divider>
      <el-table
        :data="fieldList"
        max-height="240"
        border
    ...
    <!--字段配置侧边栏-->
    <el-drawer
      :visible="fieldModelVisible"
      title="字段配置"
      :size="`${width}px`"
      custom-class="process-drawer"
      append-to-body
      destroy-on-close
    >
      <el-form
      ...
      <template v-if="formFieldForm.type === 'enum'">
        ...
        >
          <span><i class="el-icon-menu" />枚举值列表：</span>
          <el-button
            type="primary"
            @click="openFieldOptionForm(null, -1, 'enum')"
          >
            添加枚举值
          </el-button>
        </p>
        ...
        >
          ...
          >
            <template #default="{ row, $index }">
              <el-button
                type="text"
                @click="openFieldOptionForm(row, $index, 'enum')"
              >
                编辑
              </el-button>
              <el-divider direction="vertical" />
      ...
      <p
        ...
        class="listener-filed__title"
      >
        <span><i class="el-icon-menu" />约束条件列表：</span>
        <el-button
          type="primary"
      ...
      >
        ...
        >
          <template #default="{ row, $index }">
            <el-button
              type="text"
              @click="openFieldOptionForm(row, $index, 'constraint')"
            >
              编辑
            </el-button>
            <el-divider direction="vertical" />
      ...
      <p
        ...
        class="listener-filed__title"
      >
        <span><i class="el-icon-menu" />字段属性列表：</span>
        <el-button
          type="primary"
          @click="openFieldOptionForm(null, -1, 'property')"
        >
          添加属性
        </el-button>
      </p>
      ...
      >
        ...
        >
          <template #default="{ row, $index }">
            <el-button
              type="text"
              @click="openFieldOptionForm(row, $index, 'property')"
            >
              编辑
            </el-button>
            <el-divider direction="vertical" />
    ...
    >
      ...
      <template #footer>
        <el-button @click="fieldOptionModelVisible = false">
          取 消
        </el-button>
        <el-button
          type="primary"
          @click="saveFieldOption"
        >
          确 定
        </el-button>
      </template>
  ...
  methods: {
    ...
    openFieldForm(field, index) {
      ...
      if (index !== -1) {
        ...
        this.$set(
          this.formFieldForm,
          'typeType',
          !this.fieldType[field.type] ? 'custom' : field.type
        )
        // 初始化枚举值列表
        field.type === 'enum' &&
          (this.fieldEnumList = JSON.parse(
            JSON.stringify(FieldObject?.values || [])
          ))
        // 初始化约束条件列表
        this.fieldConstraintsList = JSON.parse(
          JSON.stringify(FieldObject?.validation?.constraints || [])
        )
        // 初始化自定义属性列表
        this.fieldPropertiesList = JSON.parse(
    ...
    saveField() {
      ...
      if (this.fieldPropertiesList && this.fieldPropertiesList.length) {
        ...
        Field.properties = window.bpmnInstances.moddle.create(
      ...
      if (this.fieldConstraintsList && this.fieldConstraintsList.length) {
        const fieldConstraintList = this.fieldConstraintsList.map((fc) => {
          return window.bpmnInstances.moddle.create(
            `${this.prefix}:Constraint`,
            { name: fc.name, config: fc.config }
          )
        })
        Field.validation = window.bpmnInstances.moddle.create(
      ...
      if (this.fieldEnumList && this.fieldEnumList.length) {
        Field.values = this.fieldEnumList.map((fe) => {
          return window.bpmnInstances.moddle.create(`${this.prefix}:Value`, {
            name: fe.name,
            id: fe.id
          })
        })
