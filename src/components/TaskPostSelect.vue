        ...
        <template #header>
          <div class="base-card-header">
            ...
            >
              ...
              >
                <el-radio-button label="allDeptPost">
                  全部
                </el-radio-button>
                <el-radio-button label="selfDeptPost">
                  本部门
                </el-radio-button>
                <el-radio-button label="upDeptPost">
                  上级部门
                </el-radio-button>
                <el-radio-button label="deptPost">
                  指定部门
                </el-radio-button>
              </el-radio-group>
              <el-cascader
                v-show="formData.deptType === 'deptPost'"
              ...
              />
              <!--
              <el-select v-model="formData.deptId" placeholder="选择部门" v-show="formData.deptType === 'deptPost'" style="margin-left: 10px;">
...
export default {
    ...
    getValidDeptPostList() {
      if (this.formData.deptType !== 'deptPost') {
        ...
        return (this.deptPostList || []).filter((item) => {
          return (
            item.deptId ===
            (Array.isArray(this.formData.deptId)
              ? this.formData.deptId[this.formData.deptId.length - 1]
  ...
  methods: {
    ...
    onAddPostClick() {
      this.onCancel(
        true,
        this.selectPost.map((item) => {
          return {
            id: `${this.formData.deptType}__${item.deptPostId || item.postId}`,
            deptType: this.formData.deptType,
            deptPostId:
              this.formData.deptType === 'deptPost'
                ? item.deptPostId
                : undefined,
            postId:
              this.formData.deptType === 'deptPost' ? undefined : item.postId
          }
        })
      )
    }
